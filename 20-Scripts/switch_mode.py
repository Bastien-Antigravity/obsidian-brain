...
def main() -> None:
    ...
    if choice in modes:
        ...
        session_file = osPathJoin(orchestration_dir, "AI-Session-State.md")
        ...
        # 2. Atomically update AI-Session-State.md to match
        session_updated = _update_file_field(
            session_file,
            r'active-protocol:\s*".*?"',
            'active-protocol: "[[MODE-MANUAL#Mode-{0}]]"'.format(choice)
        )
        
        print("\n✅ SUCCESS: Protocol updated to {0}".format(modes[choice]))
        print("   MODE-MANUAL.md: {0}".format("Updated" if mode_updated else "No change"))
        print("   AI-Session-State.md: {0}".format("Synced" if session_updated else "Not found (manual sync needed)"))
